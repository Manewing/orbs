...
static int print_orb_disas_instr(orb_t const *orb, int idx) {
  ...
  printf("%*s [ %6.2f%% ]\r", 60, " ",
         100.0 * orb->trace[idx] / orb->trace_count);
...
static struct orbs_command orbs_commands[] = {
    {"l", "lists (sub) entities of entity, disassembly if entity is an orb",
     &orbs_cmd_ls},
    {"cd", "change/select entity (like cd in shell)", &orbs_cmd_cd},
    {"p", "print status of entity", &orbs_cmd_p},
    {"hl", "highlight a given orb, rm for removing, info for listing highlight",
     &orbs_cmd_highlight},
    {"s", "single step currently selected orb", &orbs_cmd_s},
    {"disas",
     "print disassembly of genes of selected orb, use 'disas ndc' to skip "
     "deadcode",
     &orbs_cmd_disas},
